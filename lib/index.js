...
function decodePayload(event) {
  return event.Records.map((r) => {
    var payloadStr = new Buffer(record.kinesis.data, 'base64').toString('ascii');
    return JSON.parse(r);
  });
}
...
class Mailer {
  constructor(event) {
    ...
    this.event = event;
    this.payload = decodePayload(event);
  ...
  validate() {
    ...
    this.payload.forEach((m) => {
      if (!this.emailRegex.test(m.from) || !this.emailRegex.test(m.to)) {
